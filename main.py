...
def main():
    ...
    parser.add_argument('--batch_eval', type=int, default=0, help='batch size for evaluating networks')
    ...
    if args.batch_eval == 0:
        args.batch_eval = args.batch_train
    ...
    channel, im_size, num_classes, class_names, mean, std, dsts = get_dataset(args.dataset, args.data_path)
    #if len(dsts) == 2:
        #dst_train, dst_test = dsts
        #testloader = torch.utils.data.DataLoader(dst_test, batch_size=args.batch_eval, shuffle=False, num_workers=0)
    ...
    for exp in range(args.num_exp):
        ...
        for fold in range(num_folds):
            ...
            testloader = torch.utils.data.DataLoader(dsts[test_fold], batch_size=args.batch_eval, shuffle=False, num_workers=8)
            ...
            images_all = [torch.unsqueeze(dst_train[i][0], dim=0) for dst_train in dst_trains for i in range(len(dst_train))]
            labels_all = [dst_train[i][1] for dst_train in dst_trains for i in range(len(dst_train))]
            for i, lab in enumerate(labels_all):
                indices_class[lab].append(i)
            images_all = torch.cat(images_all, dim=0).to(args.device)
            labels_all = torch.tensor(labels_all, dtype=torch.long, device=args.device)
            print("Concatenated shapes: ", images_all.shape, labels_all.shape)
            ...
            for it in range(args.Iteration+1):
                ...
                if it in eval_it_pool and it != 0:
                    ...
                    ''' visualize and save '''
                    save_name = os.path.join(args.save_path, 'vis_%s_%s_%s_%s_%dipc_exp%d_iter%d.png'%(args.method, args.dataset, args.model, args.debias_mode, args.ipc, exp, it))
                ...
                if it == args.Iteration: # only record the final results
                    ...
                    torch.save({'data': data_save, 'accs_all_exps': accs_all_exps, }, os.path.join(args.save_path, 'res_%s_%s_%s_%s_%dipc.pt'%(args.method, args.dataset, args.model, args.debias_mode, args.ipc)))
