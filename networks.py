...
class ConvNet(nn.Module):
    ...
    def forward(self, x, gt=None, debias_mode='none'):
        ...
        if debias_mode == 'rsc':
            ...
            one_hot_sparse = torch.sparse.FloatTensor(sp_i, sp_v, torch.Size([num_rois, class_num])).to_dense().requires_grad_(requires_grad=False).cuda()
            ...
            spatial_drop_num = int(HW * self.drop_f)
            ...
            th_fg_value = torch.sort(change_vector, dim=0, descending=True)[0][int(round(float(num_rois) * self.drop_b))]
        ...
        elif debias_mode == "rsc_new":
            drop_f_pct = (1 - self.drop_f) * 100
            drop_b_pct = (1 - self.drop_b) * 100
